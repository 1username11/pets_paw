    ...
    <div class="min-h-[780px] bg-white rounded-2xl p-5">
      ...
      <div>
        <el-image
          ...
          :src="randomImage?.url"
          alt="No image"
          class="w-[640px] h-[360px] mt-5 rounded-[20px]"
        />
        <div
...
const images = ref([randomImage.value])
const loading = ref(true)
const logs = ref([''])

async function addToLikes () {
  logs.value.push('Likes')
  images.value?.push(randomImage.value)
  try {
    loading.value = true
    const res = await generalService.getImage()
    randomImage.value = { ...res[0] }
  } catch (e) {
    console.log(e)
  } finally {
    loading.value = false
